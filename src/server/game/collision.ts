...
export class CollisionDetector {
   ...
   */
  processPlayersEatingPlayers(session: GameSessionState, now: number): CollisionEvent[] {
    const events: CollisionEvent[] = [];
    const state = session.getState();
    ...
    for (let i = 0; i < state.players.length; i++) {
      ...
      for (let j = i + 1; j < state.players.length; j++) {
        ...
        if (!this.circleCollide(playerA.position, playerA.collisionRadius, playerB.position, playerB.collisionRadius)) {
          continue;
        }
        ...
        console.log(`[Collision] Eater: ${eater.nicknameDisplay} (XP: ${eater.xp}) vs ${target.nicknameDisplay} (XP: ${target.xp})`);
   ...
   */
  processPowerupCollisions(session: GameSessionState, now: number): CollisionEvent[] {
    const events: CollisionEvent[] = [];
    const state = session.getState();
    ...
    for (const player of state.players) {
      ...
      for (const powerup of state.powerups) {
        ...
        // Check collision
        if (this.circleCollide(player.position, player.collisionRadius, powerup.position, powerup.collisionRadius)) {
