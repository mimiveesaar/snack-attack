...
**Feature Branch**: `006-fish-eat-sfx`  
...
- Q: Which fish-eaten events should trigger the crunch sound? → A: Play on every fish-eaten event (any eater: player, NPC, or other fish).
- Q: How should overlapping crunch sounds be handled during rapid events? → A: Allow overlap with no cap.
- Q: Which audio settings control the crunch sound? → A: Use the existing global sound setting and volume controls.
- Q: What should happen if the crunch sound asset is missing? → A: No fallback; fail silently and continue gameplay.
- Q: Should sound events be logged or tracked? → A: Do not record analytics/logs for this sound event.
...
- Multiple fish are eaten within the same second with overlapping crunch sounds.
- The crunch audio asset fails to load or is unavailable (no fallback; gameplay continues).
...
- **FR-001**: System MUST play the provided crunch sound once for every fish-eaten event (any eater: player, NPC, or other fish) during an active match when sound is enabled.
...
- **FR-007**: System MUST fail silently when the crunch sound asset is unavailable (no fallback sound).
- **FR-008**: System MUST NOT emit analytics or logs specifically for crunch sound playback.
- **FR-006**: System MUST use existing global sound settings (on/off and volume) to control crunch sound playback.
